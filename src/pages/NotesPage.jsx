...
function NotesPage({ user, onLogout, onShowSettings, onBackToHome, notes, currentNoteId, onSwitchNote, onEditNote, onDeleteNote, onCreateNewNote }) {
  ...
  useEffect(() => {
    ...
    if (currentNoteId && user?.id) {
      const saved = localStorage.getItem(`corvusNoteImages_${user.id}_${currentNoteId}`)
      setImageMap(saved ? JSON.parse(saved) : {})
    } else {
      setImageMap({})
    }
  }, [currentNoteId, currentNote])

  // 当 imageMap 变化时，持久化到 localStorage
  useEffect(() => {
    if (!currentNoteId || !user?.id) return
    if (Object.keys(imageMap).length > 0) {
      localStorage.setItem(`corvusNoteImages_${user.id}_${currentNoteId}`, JSON.stringify(imageMap))
    }
  }, [imageMap, currentNoteId, user?.id])
